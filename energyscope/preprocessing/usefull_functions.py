...
# TODO: move to run_energyscope.py ?
# Function to run ES from python
def run_ES(config):

    cs = f"{config['case_studies_dir']}/"
    case_study_dir = cs + config["case_study_name"]

    make_dir(f"{config['temp_dir']}/output")
    make_dir(f"{config['temp_dir']}/output/hourly_data")
    make_dir(f"{config['temp_dir']}/output/sankey")

    # running ES
    logging.info('Running EnergyScope')
    try:
        run(f"{config['AMPL_path']}/ampl {config['ES_path']}/ESTD_main.run", shell=True, check=True)
