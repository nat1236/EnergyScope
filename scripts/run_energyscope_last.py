...
if __name__ == '__main__':
    ...
    config = {'case_study': 'nor_PV_first', # Name of the case study. The outputs will be printed into : config['ES_path']+'\output_'+config['case_study']
    ...
    # specify the configuration
    config2 = {'case_study': 'nor_CCGT_first',# Name of the case study. The outputs will be printed into : config['ES_path']+'\output_'+config['case_study']
          'printing': True,  # printing the data in ETSD_data.dat file for the optimisation problem
    ...
    for i in range(5):  #pour l'instant 5 itérations
        ...
        otherdem = pd.read_csv(path_dat / 'ESTD_other.dat', sep='\t', skiprows=1, index_col=0)
        end_uses = outputs['var_END_USES'].pivot(index='Hour', columns='TD', values='ELECTRICITY')
        ...
        otherdem2 = pd.read_csv(path_dat2 / 'ESTD_other.dat', sep='\t', skiprows=1, index_col=0)
        end_uses2 = outputs2['var_END_USES'].pivot(index='Hour', columns='TD', values='ELECTRICITY')
        ...
        # qexch2 = pd.read_csv(path_dat2 / 'ESTD_qexp.dat', sep='\t', skiprows=1, index_col=0)

        config['case_study'] = 'nor_PV_' + str(i)
        es.print_data(config)
        path_dat = path / 'case_studies' / config['case_study']
        cs = path_dat / 'output' / 'hourly_data'

        config2['case_study'] = 'nor_CCGT_' + str(i)
        es.print_data(config2)
        path_dat2 = path / 'case_studies' / config2['case_study']
        ...
        # EXCHANGE PART
        # cexch.replace(value=es.compute_cost(cexch,Qexch,pivot_mc2,lcoe),regex=True, inplace=True) #pivot_mc2  #marginal_cost2
        cexch = pivot_mc2 #es.compute_cost(Qexch,pivot_mc2,lcoe_pv)  #pivot_mc2
        ...
        cexch2 = pivot_mc #es.compute_cost(Qexch2,pivot_mc,lcoe_pv) #pivot_mc #marginal_cost
        ...
        # otherdem = end_uses2
        # with open(path_dat / 'ESTD_other.dat', mode='w', newline='') as td_file:
        #     td_writer = csv.writer(td_file, delimiter='\t', quotechar=' ', quoting=csv.QUOTE_MINIMAL)
        #     td_writer.writerow(['param other_dem:='])
        # otherdem = es.ampl_syntax(otherdem, ' ')
        # otherdem.to_csv(path_dat / 'ESTD_other.dat', sep='\t', mode='a', header=True, index=True, index_label=s,quoting=csv.QUOTE_NONE)

        # otherdem2 = end_uses
        # with open(path_dat2 / 'ESTD_other.dat', mode='w', newline='') as td_file:
        #     td_writer = csv.writer(td_file, delimiter='\t', quotechar=' ', quoting=csv.QUOTE_MINIMAL)
        #     td_writer.writerow(['param other_dem:='])
        # otherdem2 = es.ampl_syntax(otherdem2, ' ')
        # otherdem2.to_csv(path_dat2 / 'ESTD_other.dat', sep='\t', mode='a', header=True, index=True, index_label=s,quoting=csv.QUOTE_NONE)
        ...
        # dict_elec2 = es.read_layer(config2['case_study'], 'layer_ELECTRICITY')
        # yearly2 = es.from_td_to_year(dict_elec2, to_use)
        # yearly2c = yearly2.loc[:, yearly2.sum().abs() > 0.0]
        # # yearly2 = yearly2.loc[:, ['CCGT', 'Q_exch', 'END_USE']]
        # yearly2c.plot(title='Yearly layer ELEC (>0)')

        # yearlyccgt = yearly2.loc[:, yearly2.sum().abs() > 1.0]
        # yearlyccgt.plot(title='Yearly layer (>1)')
